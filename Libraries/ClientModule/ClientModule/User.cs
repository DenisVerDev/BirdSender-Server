...
namespace ClientModule
{
    ...
    public class User
    {
        ...
        public cm::Channel.ChannelHandler ChannelHandler = null;

        public static List<cm::Channel> GlobalChannels { get; set; }

        private bool Sync { get; set; }

        public User(dm::User user)
        {
        ...
        public void SubscribeToChannel(cm::Channel channel, bool iscreator)
        {
            ChannelHandler = new cm::Channel.ChannelHandler(NewMessage);
        ...
        private void NewAudio(cm::Channel channel, byte[] data, bool isend, string name)
        {
        ...
        private void NewImage(cm::Channel channel, byte[] data, bool isend, string name)
        {
            if (IsOline && IsConnectedToStream.Key.Equals(channel.Address))
            {
                if (Username != name)
        ...
        private void NewMessage(mm::Message msg, cm::Channel channel, bool sync)
        {
            ...
            else if (mm::MessageSerializer.IsCorrectServiceMessage(msg.Text))
            {
        ...
        public ResultCodes GetNewMessages(string address)
        {
            ...
            {
                Sync = false;
                cm::Channel channel = GlobalChannels.Where(x => x.Address == address).FirstOrDefault();
                if (channel != null)
                {
                    List<mm::Message> messages = null;
                    bool isend = channel.GetNewMessages(this.Username, out messages);
                    foreach (mm::Message msg in messages)
                    {
        ...
        public void UnSubscribeFromChannel(cm::Channel channel)
        {
            channel.MessageCameOut -= ChannelHandler;
            channel.AudioCameOut -= NewAudio;
            channel.ImageCameOut -= NewImage;
        ...
        public void DisconnectFromUser()
        {
            ...
            using (dm::MessangerDataContext db = new dm::MessangerDataContext())
            {
                ...
                dm::User user = db.Users.Where(x => x.Username == this.Username).FirstOrDefault();
                if (user != null)
                {
